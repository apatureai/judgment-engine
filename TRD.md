...
- **Phased, all behind one adapter:** DashScope (`qwen3-vl-plus` deep / `qwen3-vl-flash` triage, OpenAI-compatible) for v1; self-host as the deferred scale/residency path (guided decoding, continuous batching, prefix caching, FP8, GPU warm-pool); a fine-tuned VLM as the deferred owned judge. The adapter stamps `model`/`promptVersion`. Both self-host servers expose the OpenAI-compatible API the adapter (#26/#27) already targets, so the server is a deploy choice, not a code change.
...
- **Confidence calibration (`CalibrationReportV1`, implemented July 13, 2026; #107/#160).** `@engine/eval` serializes the post-hoc map, ECE/Brier uncertainty, reliability bins, risk/coverage, false-block bounds, valid-reference rate, repository/team/time split policy, required visual-text-conflict cohort, sample counts, thresholds, validity window, and model/prompt/engine/capture/rubric identities. `model_prompt_registry` binds the exact report id/hash; blocking promotion additionally requires sufficient current evidence and release-policy attestation. Serving reconstructs the map from the artifact before filtering/aggregation. Historical raw numbers remain parseable but unavailable without report provenance. Sources: https://arxiv.org/html/2508.06225v2 ; https://arxiv.org/pdf/2509.25532 (accessed June 21, 2026).
...
- **Eval-gated promotion:** `model_prompt_registry` versions every model/prompt and `model_prompt_calibration_bindings` binds its rubric plus exact report. CI blocks on eval failure; blocking mode additionally fails closed on missing, stale, insufficient, unattested, or identity/manifest-mismatched reports. Rollback restores the prior entry and its binding. Weekly production injected-defect canaries. Shadow/canary rollout deferred.

## 10. Feedback & Learning (E5)
...
- **Owned judge (deferred):** ORPO fine-tune on (preferred, rejected) finding pairs, promoted only behind the eval gate, swapped in behind the model adapter. Per-tenant LoRA deferred.
...
No `contents: write`; the model judges, never edits or drives. SSRF hardening with DNS-rebind checks; deny internal/metadata/link-local egress, allow public assets so pages render honestly. Screenshots encrypted at rest (SSE-KMS); tenant-scoped keys with per-repo data keys (shared CMK by default, per-tenant CMK where configured, per-repo DEK always — never per-repo KMS keys). Auth storageState encrypted, origin-scoped, off on fork PRs. Prompt-injection defenses for DOM text (delimiters) and rendered screenshot text (the schema-constrained output is the backstop; eval canaries include rendered-text attacks). Retention is policy-configurable per tenant (ephemeral by default). Enterprise residency via in-VPC self-host.
...
- **EM4 - Feedback & learning:** feedback signals, rater weighting, per-repo memory, consent/PII gate, DVC-versioned export.
