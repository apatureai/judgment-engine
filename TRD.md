...
- **Phased, all behind one adapter:** DashScope (`qwen3-vl-plus` deep / `qwen3-vl-flash` triage, OpenAI-compatible) for v1; self-host as the act-2 lever (guided decoding, continuous batching, prefix caching, FP8, GPU warm-pool); a fine-tuned Qwen3-VL as the act-3 owned judge. The adapter stamps `model`/`promptVersion`. Both self-host servers expose the OpenAI-compatible API the adapter (#26/#27) already targets, so the server is a deploy choice, not a code change.
