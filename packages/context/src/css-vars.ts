...
/**
 * A whole-value CSS `var()` reference — `var(--ref)` or `var(--ref, fallback)`.
 * Only a value that IS a single reference is resolved (the dominant design-token
 * pattern, e.g. `--button-bg: var(--color-brand)`); a `var()` embedded in a larger
 * value (`0 1px var(--c)`) is intentionally left as-is.
 */
 ...
 * fallback when one is given (resolved too), else the value is left literal. A
 * reference cycle is left as its original literal — never looped.
 */
...
function resolveScope(map: TokenMap, scope: TokenMap): TokenMap {
  const out: TokenMap = {};
