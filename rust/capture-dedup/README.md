...
`cargo test` (11 tests: golden sync + cross-checks, metric properties via LCG
sampling, near-dup/separation behavior, panics on bad dims) · `cargo clippy
--all-targets` clean · `cargo fmt --check` clean · TS mirror:
`pnpm exec vitest run packages/capture/test/dedup-golden.test.ts` (7 tests) · a
